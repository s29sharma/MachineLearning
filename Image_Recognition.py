...
from lr_utils import load_dataset #-- lack of dataset to run it here
...
#
# Loading the data by running the following code.
...
# After preprocessing, we will end up with train_set_x and test_set_x (the labels train_set_y and
...
# numpy-array of shape (num_px $*$ num_px $*$ 3, 1). After this, our training (and test) dataset
# is a numpy-array where each column represents a flattened image. There should be m_train (
# respectively m_test) columns.
#
# **Exercise:** Reshape the training and test data sets so that images of size (num_px, num_px, 3)
# are flattened into single vectors of shape (num\_px $*$ num\_px $*$ 3, 1).
#
# A trick when you want to flatten a matrix X of shape (a,b,c,d) to a matrix X_flatten of shape
# (b$*$c$*$d, a) is to use:
# ```python
...
# train_set_x_flatten = train_set_x_orig.reshape(train_set_x_orig.shape[1]*train_set_x_orig.shape[2]*3
...
# test_set_x_flatten = test_set_x_orig.reshape(test_set_x_orig.shape[1]*test_set_x_orig.shape[2]*3
...
# it is simpler and more convenient and works almost as well to just divide every row of the dataset by
...
#ToDo: Look why is our case we had to do something like [ (m_train,num_px*num_px*3).T ] i.e we took a transpose of the whole thing
# "Standardize" the data3 - General Architecture of the learning algorithm
...
#       ' sigmoid(wTx+b)=11+e−(wTx+b)sigmoid(wTx+b)=11+e−(wTx+b)  ' \
...
# dim in this case will be the shape of training data we will use. That is it is the
# shape of the training data or the number of dependent variables we have in total
...
#
# **Exercise:** Implement a function `propagate()` that computes the cost function
# and its gradient.
#
# **Hints**:
#
...
# - You calculate the cost function: $J = -\frac{1}{m}\sum_{i=1}^{m}y^{(i)}\log(a^{(i)})+
# (1-y^{(i)})\log(1-a^{(i)})$
#
...
# Exercise: Write down the optimization function. The goal is to learn  ww  and
# bb  by minimizing the cost function  JJ . For a parameter  θθ ,
# the update rule is  θ=θ−α dθθ=θ−α dθ , where  αα  is the learning rate.
...
#
# 2. Convert the entries of a into 0 (if activation <= 0.5) or
# 1 (if activation > 0.5), stores the predictions in a vector `Y_prediction`.
# If you wish, you can use an `if`/`else` statement in a `for` loop (though there is
# also a way to vectorize this).
...
# Comment: Training accuracy is close to 100%. This is a good sanity check: your model
# is working and has high enough capacity to fit the training data. Test error is 68%.
# It is actually not bad for this simple model,
...
# for example by using regularization. Using the code below (and changing the index variable)
# you can look at predictions on pictures of the test set.
...
# You might see that the training
# set accuracy goes up, but the test set accuracy goes down. This is called overfitting.

# 6 - Further analysis (optional/ungraded exercise)
# Congratulations on building your first image classification model.
# Let's analyze it further, and examine possible choices for the learning rate αα.
#
...
# If the learning rate is too large we may
# "overshoot" the optimal value. Similarly, if it is too small we will need too
# many iterations to converge to the best values.
...
# It may even diverge (though in this example, using 0.01 still eventually
# ends up at a good value for the cost).
# A lower cost
# doesnt mean a better model.
# You have to check if there is possibly overfitting.
...
# Feel free also to try different values than the three we have initialized the
# learning_rates variable to contain, and see what happens.
